...
-- 2a. You need to find the ID number, first name, and last name of an actor, first name, "Joe." 
...
-- 3a. Create a column in the table actor named description and use the data type BLOB. (Binary large Object)
ALTER TABLE actor
...
-- 4b. List last names of actors and the number of actors who have that last name that are shared by at least two actors
SELECT last_name, COUNT(*) as 'Last Name Count'   
FROM actor  
GROUP BY last_name
HAVING COUNT(*) >=2; 
...
-- 4d. Turns out that GROUCHO was the correct name! In a single query, if first name is currently HARPO, change it to GROUCHO.
UPDATE actor 
...
-- 6c. List each film and the number of actors. Use tables film_actor and film. Use inner join.
...
-- 6e. Using the tables payment and customer and JOIN, list the total paid by each customer. List alphabetically by last name:
...
-- 7h. List the top five genres in gross revenue in descending order. (Hint: use the following tables: category, film_category, inventory, payment, and rental.)
